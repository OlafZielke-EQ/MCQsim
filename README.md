...
Following below are the results of [_strong scaling_](https://hpc-wiki.info/hpc/Scaling) tests that were done on [Shaheen3@KAUST](https://www.hpc.kaust.edu.sa). For these tests, we used two intersecting faults (parameterized by 10k, 50k, 100k, and 200k elements) with average slip rates of ~3mm/yr and computed 5-kyr-long catalogs. We show these scaling tests here a) to showcase _MCQsim_ performance on our HPC facilities, and b) to provide users with first-order guidence on simulation preparation (e.g., CPU number as a function of fault element number i.e., fault system size). Below, we show scaling in terms of speed up factor (relative to single-CPU simulation) and effciency. The simulations indicate that the speed up factor closely follows the ideal case, before scaling begins to come less efficient (at different CPU numbers for the different fault element numbers). The "odd" shapes in effciency curves for the 10k and 50k element simulations reflect the internal CPU organization within a single node of Shheen3 (each node consisting of 192 CPUs, single-node indicated by gray area). Given these scaling tests we suggest to use: 6-12 CPUs for 10k elements, 24-48 CPUs for 50k elements, 384-768 CPUs for 100k elements, and 768 to 1536 CPUs for 200k elements.
