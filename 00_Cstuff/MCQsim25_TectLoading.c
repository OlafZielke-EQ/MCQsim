...
void TectonicLoading(int _avt8k06, int _20n8tte, const struct _r1yxr2d *restrict _5zja8dz, const struct _q0oxfq8 *restrict _z3nx39r, const int *restrict _b6gs8g8, const int *restrict _zfflb7w, const unsigned int *restrict _xo100vh, const unsigned int *restrict _g1ved32, float *restrict _3n3bzt9, float *restrict _3p91i3v, float *restrict _m7pyxdw, const float *restrict _rcpprcj, const unsigned short int *restrict _iob0nlw, const unsigned short int *restrict _iymyrsv, const unsigned short int *restrict _btkf6xd, const unsigned short int *restrict _9f77lu8, const unsigned short int **restrict _3b9so0u, const unsigned short int **restrict _1ajo091, const unsigned short int **restrict _86lthet, const unsigned short int **restrict _op767j2, const float **restrict _0dtirc9, const float **restrict _qk4ltah, const float **restrict _h61iaoi, const float **restrict _nf74t60, const float **restrict _mrunjdu, const float **restrict _jbc2sws, const float **restrict _4xkss0k, const float **restrict _t997e4u, const float **restrict _ge2j51b, const float **restrict _sh3tow4, const float **restrict _pqvl00e)
{
    ...
    const float *_mybd2qe = (const float *)_hj3muur;
    const float *_7yt2m6t = (const float *)_z2c9ros;
    const float *_xf7jxjn = (const float *)_qxmcthp;
    const float *_9yo18pa = (const float *)_32orwix;
    clock_gettime(_CLOCK_REALTIME, &_7dknncp);
    unsigned int _9ufu6py = 0u;
    if ( _9ufu6py == 1u)
    { _ybxkpm2 = 0u;
        for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
        { _4pa8hye[_ybxkpm2*3 +0] = (float)(_xo100vh[i]);
            _4pa8hye[_ybxkpm2*3 +1] = -1.0f*_3n3bzt9[i*16 +13]*_3n3bzt9[i*16 +8];
            _4pa8hye[_ybxkpm2*3 +2] = -1.0f*_3n3bzt9[i*16 +14]*_3n3bzt9[i*16 +8];
            _ybxkpm2 += 1u;
        }
        ...
        { __builtin___memset_chk (_qxmcthp, 0, 2*_iob0nlw[i]*sizeof(float), __builtin_object_size (_qxmcthp, 0));
            ...
            { _h9ier7u[0] = (unsigned int)_mybd2qe[j*3 +0];
                _h9ier7u[1] = _3b9so0u[i][_h9ier7u[0]] *2;
                _h9ier7u[0] = j*3;
                _qxmcthp[_h9ier7u[1] +0] += _mybd2qe[_h9ier7u[0] +1];
                _qxmcthp[_h9ier7u[1] +1] += _mybd2qe[_h9ier7u[0] +2];
            }
            _3n3bzt9[i*16 +9] = cblas_sdot(2*_iob0nlw[i], _xf7jxjn, 1, _0dtirc9[i], 1);
            _3n3bzt9[i*16 +10] = cblas_sdot(2*_iob0nlw[i], _xf7jxjn, 1, _qk4ltah[i], 1);
            _3p91i3v[i*8 +5] = sqrtf(_3n3bzt9[i*16 +13]*_3n3bzt9[i*16 +13] +_3n3bzt9[i*16 +14]*_3n3bzt9[i*16 +14]);
            _3n3bzt9[i*16 +11] = _3n3bzt9[i*16 +13];
            _3n3bzt9[i*16 +12] = _3n3bzt9[i*16 +14];
        }
    }
    else
    { for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
        { _9p3jpxg[2] = sqrtf(_3n3bzt9[i*16 +13]*_3n3bzt9[i*16 +13] +_3n3bzt9[i*16 +14]*_3n3bzt9[i*16 +14]);
            _iwvhdk8[0] += (0.5*(_3n3bzt9[i*16 +5] + _3n3bzt9[i*16 +6]) );
            _iwvhdk8[1] += (_9p3jpxg[2] <= 0.0f)*0.0f + (_9p3jpxg[2] > 0.0f)*_9p3jpxg[2];
            _iwvhdk8[2] += (_9p3jpxg[2] <= 0.0f)*0.0f + (_9p3jpxg[2 ]> 0.0f)*1.0f;
            _3n3bzt9[i*16 +9] += (-1.0f*_3n3bzt9[i*16 +13] *_3n3bzt9[i*16 +5]);
            _3n3bzt9[i*16 +10]+= (-1.0f*_3n3bzt9[i*16 +14] *_3n3bzt9[i*16 +6]);
        }
        MPI_Allreduce((void *) -1, _iwvhdk8, 3, ((MPI_Datatype)0x4c00040a), (MPI_Op)(0x58000003), ((MPI_Comm)0x44000000));
        _e3nwxuj = fabsf(1000.0f *_5zja8dz->_at9mx2x / (_iwvhdk8[0]/(float)_z3nx39r->_jp61lfc));
        _9p3jpxg[0] = (_iwvhdk8[2] <= 0.0f)*-1.0f + (_iwvhdk8[2] > 0.0f)*_iwvhdk8[2];
        _rrpzlk3 = (_iwvhdk8[2] <= 0.0f)*0.0f + (_iwvhdk8[2] > 0.0f)* (_iwvhdk8[1]/_9p3jpxg[0]);
        if (_z3nx39r->_urvqt3g > 0u)
        {
            _jg2y8rg = 0u;
            { for (i = 0u; i < _zfflb7w[_20n8tte]; i++)
                {
                    _iq2fa39 += ((_m7pyxdw[i*16 +5] + _m7pyxdw[i*16 +6] + _m7pyxdw[i*16 +7])/3.0f); _9p3jpxg[0] = -1.0f*_m7pyxdw[i*16 +9];
                    _9p3jpxg[1] = -1.0f*_m7pyxdw[i*16 +10]; _9p3jpxg[2] = -1.0f*_m7pyxdw[i*16 +11];
                    if ((fabsf(_9p3jpxg[0]) + fabsf(_9p3jpxg[1]) + fabsf(_9p3jpxg[2])) > 0.0f)
                    { _wd1xmxb[_jg2y8rg*4 +0] = (float)(_g1ved32[i]);
                        _wd1xmxb[_jg2y8rg*4 +1] = _9p3jpxg[0]*_m7pyxdw[i*16 +8];
                        _wd1xmxb[_jg2y8rg*4 +2] = _9p3jpxg[1]*_m7pyxdw[i*16 +8];
                        _wd1xmxb[_jg2y8rg*4 +3] = _9p3jpxg[2]*_m7pyxdw[i*16 +8];
                        _jg2y8rg += 1u;
            } } }
            MPI_Allreduce((void *) -1, &_iq2fa39, 1, ((MPI_Datatype)0x4c00040a), (MPI_Op)(0x58000003), ((MPI_Comm)0x44000000));
            _iq2fa39 = fabsf(1000.0f *_5zja8dz->_at9mx2x / (_iq2fa39/(float)_z3nx39r->_urvqt3g));
            ...
            { __builtin___memset_chk (_32orwix, 0, 3*_iymyrsv[i]*sizeof(float), __builtin_object_size (_32orwix, 0));
                ...
                { _h9ier7u[0] = (unsigned int)_7yt2m6t[j*4 +0];
                    _h9ier7u[1] = _1ajo091[i][_h9ier7u[0]] *3;
                    _h9ier7u[0] = j*4;
                    _32orwix[_h9ier7u[1] +0] += _7yt2m6t[_h9ier7u[0] +1];
                    _32orwix[_h9ier7u[1] +1] += _7yt2m6t[_h9ier7u[0] +2];
                    _32orwix[_h9ier7u[1] +2] += _7yt2m6t[_h9ier7u[0] +3];
                }
                _3n3bzt9[i*16 +9] += cblas_sdot(3*_iymyrsv[i], _9yo18pa, 1, _nf74t60[i], 1);
                _3n3bzt9[i*16 +10]+= cblas_sdot(3*_iymyrsv[i], _9yo18pa, 1, _mrunjdu[i], 1);
            }
        }
        { unsigned int _i4w6ozx = 0u;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _3n3bzt9[i*16 +0] = _3n3bzt9[i*16 +9]; _3n3bzt9[i*16 +1] = _3n3bzt9[i*16 +10];
                _3n3bzt9[i*16 +11] = 0.0f; _3n3bzt9[i*16 +12] = 0.0f;
                _9p3jpxg[0] = sqrtf(_3n3bzt9[i*16 +9]*_3n3bzt9[i*16 +9] + _3n3bzt9[i*16 +10]*_3n3bzt9[i*16 +10]);
                _i4w6ozx = (_9p3jpxg[0] <= 1.19209290e-7F)*_i4w6ozx + (_9p3jpxg[0] > 1.19209290e-7F)*1u;
            }
            MPI_Allreduce((void *) -1, &_i4w6ozx, 1, ((MPI_Datatype)0x4c000406), (MPI_Op)(0x58000001), ((MPI_Comm)0x44000000));
            if (_i4w6ozx == 0u) { fprintf(__stdoutp,"no load was defined. abort \n"); exit(123); }
        }
        for (k = 0u; k < _hhpbmyo; k++)
        {
            _ybxkpm2 = 0u;
            _rgb93wk = 0u;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _9p3jpxg[0] = -_3n3bzt9[i*16 +0]/_3n3bzt9[i*16 +5]; _9p3jpxg[1] = -_3n3bzt9[i*16 +1]/_3n3bzt9[i*16 +6];
                _3n3bzt9[i*16 +11] += _9p3jpxg[0]; _3n3bzt9[i*16 +12] += _9p3jpxg[1];
                _4pa8hye[_ybxkpm2*3 +0] = (float)(_xo100vh[i]);
                _4pa8hye[_ybxkpm2*3 +1] = _9p3jpxg[0]*_3n3bzt9[i*16 +8];
                _4pa8hye[_ybxkpm2*3 +2] = _9p3jpxg[1]*_3n3bzt9[i*16 +8];
                _ybxkpm2 += 1u;
            }
            _jg2y8rg = 0u;
            _kb700uc = 0u;
            for (i = 0u; i < _zfflb7w[_20n8tte]; i++)
            { _9p3jpxg[0] = -_m7pyxdw[i*16 +0]/_m7pyxdw[i*16 +5]; _9p3jpxg[1] = -_m7pyxdw[i*16 +1]/_m7pyxdw[i*16 +6]; _9p3jpxg[2] = -_m7pyxdw[i*16 +2]/_m7pyxdw[i*16 +7];
                _wd1xmxb[_jg2y8rg*4 +0] = (float)(_g1ved32[i]);
                _wd1xmxb[_jg2y8rg*4 +1] = _9p3jpxg[0]*_m7pyxdw[i*16 +8];
                _wd1xmxb[_jg2y8rg*4 +2] = _9p3jpxg[1]*_m7pyxdw[i*16 +8];
                _wd1xmxb[_jg2y8rg*4 +3] = _9p3jpxg[2]*_m7pyxdw[i*16 +8];
        ...
        { __builtin___memset_chk (_iwvhdk8, 0, 3*sizeof(float), __builtin_object_size (_iwvhdk8, 0));
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _9p3jpxg[2] = fabsf(_3n3bzt9[i*16 +13]) + fabsf(_3n3bzt9[i*16 +14]);
                _9p3jpxg[3] = sqrtf(_3n3bzt9[i*16 +11]*_3n3bzt9[i*16 +11] +_3n3bzt9[i*16 +12]*_3n3bzt9[i*16 +12]);
                _iwvhdk8[1] += (_9p3jpxg[2] <= 0.0f)*0.0f + (_9p3jpxg[2] > 0.0f)*_9p3jpxg[3];
                _iwvhdk8[2] += (_9p3jpxg[2] <= 0.0f)*0.0f + (_9p3jpxg[2] > 0.0f)*1.0f;
                _3p91i3v[i*8 +5] = _9p3jpxg[3];
            }
            MPI_Allreduce((void *) -1, &_iwvhdk8, 3, ((MPI_Datatype)0x4c00040a), (MPI_Op)(0x58000003), ((MPI_Comm)0x44000000));
            _9p3jpxg[0] = (_iwvhdk8[2] <= 0.0f)*-1.0f + (_iwvhdk8[2] > 0.0f)*_iwvhdk8[2];
            _e4xyf9i = (_iwvhdk8[2] <= 0.0f)*0.0f + (_iwvhdk8[2] > 0.0f)*(_iwvhdk8[1]/_9p3jpxg[0]);
            if ((_rrpzlk3 > 1.19209290e-7F) && (_e4xyf9i > 1.19209290e-7F)) { _9p3jpxg[0] = _rrpzlk3/_e4xyf9i; }
            else { _9p3jpxg[0] = 1.0f; }
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _3n3bzt9[i*16 +11] *= _9p3jpxg[0]; _3n3bzt9[i*16 +12] *= _9p3jpxg[0]; _3p91i3v[i*8 +5] *= _9p3jpxg[0];
            }
        }
        { _ybxkpm2 = 0u;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _4pa8hye[_ybxkpm2*3 +0] = (float)(_xo100vh[i]);
                _4pa8hye[_ybxkpm2*3 +1] = -_3n3bzt9[i*16 +11]*_3n3bzt9[i*16 +8];
                _4pa8hye[_ybxkpm2*3 +2] = -_3n3bzt9[i*16 +12]*_3n3bzt9[i*16 +8];
                _ybxkpm2 += 1u;
            }
            MPI_Allgather(&_ybxkpm2, 1, ((MPI_Datatype)0x4c000406), _o2t8fy6, 1, ((MPI_Datatype)0x4c000405), ((MPI_Comm)0x44000000));
            _gg24479[0] = 0u;
        ...
        { float _mt8vt43, _b4ldims, _9p3jpxg;
            _9p3jpxg = 0.0f;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++) { _9p3jpxg += _3p91i3v[i*8 +5]; }
            MPI_Allreduce((void *) -1, &_9p3jpxg, 1, ((MPI_Datatype)0x4c00040a), (MPI_Op)(0x58000003), ((MPI_Comm)0x44000000));
            _mt8vt43 = _9p3jpxg/(float)_z3nx39r->_jp61lfc;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _3n3bzt9[i*16 +9] /= ( ((1.0f) > (_3p91i3v[i*8 +5]/_mt8vt43)) *(1.0f) + ((1.0f) <= (_3p91i3v[i*8 +5]/_mt8vt43)) *(_3p91i3v[i*8 +5]/_mt8vt43) );
                _3n3bzt9[i*16 +10] /= ( ((1.0f) > (_3p91i3v[i*8 +5]/_mt8vt43)) *(1.0f) + ((1.0f) <= (_3p91i3v[i*8 +5]/_mt8vt43)) *(_3p91i3v[i*8 +5]/_mt8vt43) );
                _3n3bzt9[i*16 +11] /= ( ((1.0f) > (_3p91i3v[i*8 +5]/_mt8vt43)) *(1.0f) + ((1.0f) <= (_3p91i3v[i*8 +5]/_mt8vt43)) *(_3p91i3v[i*8 +5]/_mt8vt43) );
                _3n3bzt9[i*16 +12] /= ( ((1.0f) > (_3p91i3v[i*8 +5]/_mt8vt43)) *(1.0f) + ((1.0f) <= (_3p91i3v[i*8 +5]/_mt8vt43)) *(_3p91i3v[i*8 +5]/_mt8vt43) );
                _3p91i3v[i*8 +5] /= ( ((1.0f) > (_3p91i3v[i*8 +5]/_mt8vt43)) *(1.0f) + ((1.0f) <= (_3p91i3v[i*8 +5]/_mt8vt43)) *(_3p91i3v[i*8 +5]/_mt8vt43) );
            }
            _9p3jpxg = 0.0f;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++) { _9p3jpxg += _3p91i3v[i*8 +5]; }
            MPI_Allreduce((void *) -1, &_9p3jpxg, 1, ((MPI_Datatype)0x4c00040a), (MPI_Op)(0x58000003), ((MPI_Comm)0x44000000));
            _b4ldims = _9p3jpxg/(float)_z3nx39r->_jp61lfc;
            for (i = 0u; i < _b6gs8g8[_20n8tte]; i++)
            { _3n3bzt9[i*16 +9] *= (_mt8vt43/_b4ldims);
                _3n3bzt9[i*16 +10] *= (_mt8vt43/_b4ldims);
                _3n3bzt9[i*16 +11] *= (_mt8vt43/_b4ldims);
                _3n3bzt9[i*16 +12] *= (_mt8vt43/_b4ldims);
                _3p91i3v[i*8 +5] *= (_mt8vt43/_b4ldims);
            }
        }
    }
    ...
    if (_20n8tte == 0)
    { fprintf(__stdoutp,"Time taken to determine loading function (in seconds): %lf\n",_6parsg7);
        fprintf(__stdoutp,"Minimum AfterSlip (mm): %3.3f\n", _e3nwxuj);
        fprintf(__stdoutp,"\nResulting average tectonic slip-rate on faults (mm/yr): %5.5f   and  %5.5f\n",_rrpzlk3*1000.0f, _e4xyf9i*1000.0f);
    }
    free (_4pa8hye); free(_hj3muur); free (_wd1xmxb); free(_z2c9ros); free(_qxmcthp); free(_32orwix);
    return;
}
