...
void       RotGlb2Loc(float fPrvStress[9], float fStressStk[6], float fStressDip[6], float fStressNrm[6], float fStk[3], float fDip[3], float fNrm[3]);
...
{   if ( (argc  > 3 ) || (argc < 2) ) {   fprintf(stdout,"Input Error\n Please start the code in the following way:\n\n mpirun -np 4 ./MCQsimV2p5 RunParaFile.txt");   exit(10);    }
    ...
    float        *fFEvent   = calloc(16*iFOFFSET[iRANK],  sizeof *fFEvent );
    ...
    {   unsigned int uGlobPos;
        ...
        {   uGlobPos   = i + iFSTART[iRANK];
            ...
            {   fFEvent[i*16 +8]  = 0.0f;
                fFEvent[i*16 +9]  = 0.0f;
                fFEvent[i*16 +13] = 1.0E-3f*fF_temp[uGlobPos*16 +4] *cosf(fF_temp[uGlobPos*16 +5]*(M_PI/180.0f));
                fFEvent[i*16 +14] = 1.0E-3f*fF_temp[uGlobPos*16 +4] *sinf(fF_temp[uGlobPos*16 +5]*(M_PI/180.0f));
            }
            else
            {   fStressVG[0]      = 1.0E+6f*fF_temp[uGlobPos*16 +4] *sinf(fF_temp[uGlobPos*16 +5]*(M_PI/180.0f));
                fStressVG[1]      = 1.0E+6f*fF_temp[uGlobPos*16 +4] *cosf(fF_temp[uGlobPos*16 +5]*(M_PI/180.0f));
                fStressVG[2]      = 0.0f; 
                //----------------------------
                fStressVL[0]      = fF_temp[i*16 +6]*fStressVG[0] + fF_temp[i*16 +7]*fStressVG[1] + fF_temp[i*16 +8]*fStressVG[2];
                fStressVL[1]      = fF_temp[i*16 +9]*fStressVG[0] + fF_temp[i*16+10]*fStressVG[1] + fF_temp[i*16+11]*fStressVG[2];
                fStressVL[2]      = fF_temp[i*16+12]*fStressVG[0] + fF_temp[i*16+13]*fStressVG[1] + fF_temp[i*16+14]*fStressVG[2];
                //----------------------------
                fFEvent[i*16 +8]  = fStressVL[1];
                fFEvent[i*16 +9]  = fStressVL[2];
    ...
    {   //--------------------------------------------------------------------------------
        ...
        {   timer0 = clock();
            ...
            float fNrm[3],   fStk[3],   fDip[3],  fTempVect[3],   fPrvStress[9],   fTstStress[9];
            ...
            {   //--------------------------------
                ...
                //--------------------------------
                uGlobPos = i+ iFSTART[iRANK];
                memcpy(fRcv, &fF_temp[uGlobPos*16 +0], 3u*sizeof(float));       memcpy(fNrm, &fF_temp[uGlobPos*16 +6], 3u*sizeof(float)); 
                memcpy(fStk, &fF_temp[uGlobPos*16 +9], 3u*sizeof(float));       memcpy(fDip, &fF_temp[uGlobPos*16+12], 3u*sizeof(float));
                //-------------------------------------------------------
                uKh_FFcnt[i]  = 0u;
                uUsdNum       = 0u;
                ...
                {   //--------------------------------
                    ...
                    {   memcpy(fP1, &fFV_temp[uF_temp[uSrcElem*7 +0]*4 +0], 3u*sizeof(float));
                        ...
                        //-------------------------------------------------------
                        ScaleStress6(fStressStk, (fDistF/fUnitSlipF));
                        ScaleStress6(fStressDip, (fDistF/fUnitSlipF));
                        ScaleStress6(fStressNrm, (fDistF/fUnitSlipF));
                        //-------------------------------------------------------
                        RotGlb2Loc(fPrvStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                        //-------------------------------------------------------
                        if (uGlobPos == uSrcElem)
                        {   fFEvent[i*16 +4] = fPrvStress[0];
                            fFEvent[i*16 +5] = fPrvStress[4];
                            fFEvent[i*16 +6] = fPrvStress[8];
                            fFRef[i*14 +2]   = MAX(fabs(fFEvent[i*16 +4]),fabs(fFEvent[i*16 +5]));
                            fFRef[i*14 +2]   = MAX(fFRef[i*14 +2],        fabs(fFEvent[i*16 +6]));
                            //--------------------------------------------------------------------
                            fTemp0 = ran0(&lSeed);      fTemp0 = fTemp0*2.0f -1.0f;
                            fFEvent[i*16 +7] = fFRef[i*14 +7] + (fFRef[i*14 +8] * fTemp0);
                            ...
                            uFEvent[i*5 +0]  = (fTemp1 > fFEvent[i*16 +7])*1u + (fTemp1 <= fFEvent[i*16 +7])*uFEvent[i*5 +0];
                            //---------------------------------
                            fTemp0 = fFFric[i*6 +1]*-1.0f*fFRef[i*14 +1] + fFRef[i*14 +2]*fFEvent[i*16 +7];
                            ...
                            //---------------------------------
                            fFEvent[i*16 +10]= (fFFric[i*6 +1] - fFFric[i*6 +2])*-1.0f*fFRef[i*14 +1];
                            //--------------------------------------------------------------------
                        }
                        //---------------------------------------------
                    ...
                    {   uPrvBrLv -= 1u;
                        ...
                        {   //----------------------------
                            ...
                            {   //---------------------------------------
                                ...
                                //-------------------------------------------------------
                                ScaleStress6(fStressStk, (fDistF/fUnitSlipF));          ScaleStress6(fStressStk, (1.0f/fKh_FFBrch[uPrvBrId*21 +13]));
                                ScaleStress6(fStressDip, (fDistF/fUnitSlipF));          ScaleStress6(fStressDip, (1.0f/fKh_FFBrch[uPrvBrId*21 +13]));
                                ScaleStress6(fStressNrm, (fDistF/fUnitSlipF));          ScaleStress6(fStressNrm, (1.0f/fKh_FFBrch[uPrvBrId*21 +13]));
                                //-------------------------------------------------------
                                RotGlb2Loc(fPrvStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                                //-------------------------------------------------------
                                uTemp0 = 1u;
                                ...
                                {   uTstBrId = uF_OTPrtChld[uPrvBrId*9 +(j+1)];
                                    ...
                                    //-------------------------------------------------------
                                    ScaleStress6(fStressStk, (fDistF/fUnitSlipF));          ScaleStress6(fStressStk, (1.0f/fKh_FFBrch[uTstBrId*21 +13]));
                                    ScaleStress6(fStressDip, (fDistF/fUnitSlipF));          ScaleStress6(fStressDip, (1.0f/fKh_FFBrch[uTstBrId*21 +13]));
                                    ScaleStress6(fStressNrm, (fDistF/fUnitSlipF));          ScaleStress6(fStressNrm, (1.0f/fKh_FFBrch[uTstBrId*21 +13]));
                                    //-------------------------------------------------------
                                    RotGlb2Loc(fTstStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                                    //--------------------------------
                ...
                {   //--------------------------------
                    ...
                    {   memcpy(fP1, &fBV_temp[uB_temp[uSrcElem*4 +0]*3 +0], 3u*sizeof(float));
                        ...
                        //-------------------------------------------------------
                        ScaleStress6(fStressStk, (fDistF/fUnitSlipB));
                        ScaleStress6(fStressDip, (fDistF/fUnitSlipB));
                        ScaleStress6(fStressNrm, (fDistF/fUnitSlipB));
                        //-------------------------------------------------------
                        RotGlb2Loc(fPrvStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                        //-------------------------------------------------------
                    ...
                    {   uPrvBrLv -= 1u;
                        ...
                        {   //----------------------------
                            ...
                            {   //---------------------------------------
                                ...
                                //-------------------------------------------------------
                                ScaleStress6(fStressStk, (fDistF/fUnitSlipB));          ScaleStress6(fStressStk, (1.0f/fKh_BBBrch[uPrvBrId*21 +13]));
                                ScaleStress6(fStressDip, (fDistF/fUnitSlipB));          ScaleStress6(fStressDip, (1.0f/fKh_BBBrch[uPrvBrId*21 +13]));
                                ScaleStress6(fStressNrm, (fDistF/fUnitSlipB));          ScaleStress6(fStressNrm, (1.0f/fKh_BBBrch[uPrvBrId*21 +13]));
                                //-------------------------------------------------------
                                RotGlb2Loc(fPrvStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                                //-------------------------------------------------------
                                uTemp0 = 1u;
                                ...
                                {   uTstBrId = uB_OTPrtChld[uPrvBrId*9 +(j+1)];
                                    ...
                                    //-------------------------------------------------------
                                    ScaleStress6(fStressStk, (fDistF/fUnitSlipB));          ScaleStress6(fStressStk, (1.0f/fKh_BBBrch[uTstBrId*21 +13]));
                                    ScaleStress6(fStressDip, (fDistF/fUnitSlipB));          ScaleStress6(fStressDip, (1.0f/fKh_BBBrch[uTstBrId*21 +13]));
                                    ScaleStress6(fStressNrm, (fDistF/fUnitSlipB));          ScaleStress6(fStressNrm, (1.0f/fKh_BBBrch[uTstBrId*21 +13]));
                                    //-------------------------------------------------------
                                    RotGlb2Loc(fTstStress, fStressStk, fStressDip, fStressNrm, fStk, fDip, fNrm);
                                    //--------------------------------
            ...
            {   //--------------------------------
                ...
                memcpy(fRcv, &fB_temp[uGlobPos*16 +0], 3u*sizeof(float));       memcpy(fNrm, &fB_temp[uGlobPos*16 +6], 3u*sizeof(float)); 
                memcpy(fStk, &fB_temp[uGlobPos*16 +9], 3u*sizeof(float));       memcpy(fDip, &fB_temp[uGlobPos*16+12], 3u*sizeof(float));
                //-------------------------------------------------------
                uKh_BBcnt[i]  = 0u;
                uUsdNum       = 0u;
                memset(uBUsdEl, 0,       uBPNum*sizeof(unsigned int));
    ...
    {   float fTemp0,   fTemp1,   fTemp2;
        for (i = 0u; i < iFOFFSET[iRANK]; i++)
        {   fTemp0   = sqrtf(fFEvent[i*16 +8]*fFEvent[i*16 +8] + fFEvent[i*16 +9]*fFEvent[i*16 +9]);
    ...
    {   //--------------------------------------------------------------------------------
        ...
        {   timerI = clock() - timerI;        time_InterSeis += (((double)timerI)/CLOCKS_PER_SEC);
            ...
            {   //-----------------------
                ...
                //-----------------------
                fTemp0 = (fFFric[i*6 +1] *-1.0f *fFEvent[i*16 +2]) + fFRef[i*14 +2]*fFEvent[i*16 +7];
                fTemp0/= (-1.0f *fFEvent[i*16 +2]);
                fFEvent[i*16 +3] = MAX(fFEvent[i*16 +3], fTemp0);
                fFFric[i*6 +3]   = fFEvent[i*16 +3];
                fFFric[i*6 +5]   = fFEvent[i*16 +3];
                //-----------------------
            }
            //----------------------------------------------------------------------------
            //----------------------------------------------------------------------------
            ...
            {   uEQstillOn = 0u;
                ...
                {   fTemp2 = fFFric[i*6 +3] - (fFFric[i*6 +3] - fFFric[i*6 +2]) *(fFEvent[i*16 +12]/fFEvent[i*16 +7]);
                    ...
                    fTemp3 = fFEvent[i*16 +3] + fHealFact*(fFFric[i*6 +5] - fFEvent[i*16 +3]);
                    ...
                    fTemp3           = INTERNALCOHESION + INTERNALFRICTION*-1.0f*fFEvent[i*16 +2];
                    ...
                    fTemp2   = fFFric[i*6 +2]   *-1.0f*fFEvent[i*16 +2];
                    ...
                    {   //------------------------
                        ...
                        fTemp7 = fFEvent[i*16 +15]/sqrtf(fTemp5*fTemp5 +fTemp6*fTemp6);
            ...
            {   uEQcntr += 1u;
                ...
                {   uTemp1   = uFActivEl[i];
                    ...
                    fEQMeanVals[5] +=  sqrtf(fFTempVal[uTemp1*5 +0]*fFTempVal[uTemp1*5 +0] + fFTempVal[uTemp1*5 +1]*fFTempVal[uTemp1*5 +1]) - sqrtf(fFEvent[uTemp1*16 +0]*fFEvent[uTemp1*16 +0] + fFEvent[uTemp1*16 +1]*fFEvent[uTemp1*16 +1]);
                    ...
                    fPtchDTau[i] = sqrtf(fFTempVal[uTemp1*5 +0]*fFTempVal[uTemp1*5 +0] + fFTempVal[uTemp1*5 +1]*fFTempVal[uTemp1*5 +1]) - sqrtf(fFEvent[uTemp1*16 +0]*fFEvent[uTemp1*16 +0] + fFEvent[uTemp1*16 +1]*fFEvent[uTemp1*16 +1]);
            ...
            {   memset(lSTF_offset, 0, iSIZE*sizeof(long long int) );           memset(lSTF_start, 0, iSIZE*sizeof(long long int) );
                ...
                {   if (uFEvent[i*5 +1] != 0u) 
                    {   uTemp0 = i + iFSTART[iRANK];
                        ...
                        //----------------------------------------
                        MPI_File_write_at(fp_STF, (lSTF_start[iRANK]+lTemp0),  &fFEvent[i*16 +13],1, MPI_FLOAT,     &STATUS);           lTemp0 += 1*sizeof(float);
                        MPI_File_write_at(fp_STF, (lSTF_start[iRANK]+lTemp0),  &uFEvent[i*5 +4],  1, MPI_UNSIGNED,  &STATUS);           lTemp0 += 1*sizeof(unsigned int);
                        MPI_File_write_at(fp_STF, (lSTF_start[iRANK]+lTemp0),  &fFEvent[i*16 +14],1, MPI_FLOAT,     &STATUS);           lTemp0 += 1*sizeof(float);
                        MPI_File_write_at(fp_STF, (lSTF_start[iRANK]+lTemp0),  &uFEvent[i*5 +4],  1, MPI_UNSIGNED,  &STATUS);           lTemp0 += 1*sizeof(unsigned int);
                        //----------------------------------------
            ...
            {   if (uChgBtwEQs == 1u)
                {   for (i = 0u; i < uActElmL; i++)  
                    {   //--------------------------------------------------------------------
                        uTemp1 = uFActivEl[i]; 
                        fTemp0 = ran0(&lSeed);      fTemp0 = fTemp0*2.0f -1.0f;
                        fFEvent[uTemp1*16 +7] = fFRef[uTemp1*14 +7] + (fFRef[uTemp1*14 +8] * fTemp0);
                        ...
                        uFEvent[uTemp1*5 +0] = (fTemp1 > fFEvent[uTemp1*16 +7])*1u + (fTemp1 <= fFEvent[uTemp1*16 +7])*uFEvent[uTemp1*5 +0];
                        //---------------------------------
                        fTemp0 = fFFric[uTemp1*6 +1]*-1.0f*fFRef[uTemp1*14 +1] + fFRef[uTemp1*14 +2]*fFEvent[uTemp1*16 +7];
                        ...
                        //---------------------------------
                        fFEvent[uTemp1*16 +10] = (fFFric[uTemp1*6 +1] - fFFric[uTemp1*6 +2])*-1.0f*fFRef[uTemp1*14 +1];
                        //--------------------------------------------------------------------
                }   }
                //--------------------
                float fTempCurrFric;
                ...
                {   fFRef[i*14 +13] += sqrtf(fFEvent[i*16 +13]*fFEvent[i*16 +13] +fFEvent[i*16 +14]*fFEvent[i*16 +14]);
                    ...
                    //---------------------------------
                    fTemp0           = sqrtf(fFEvent[i*16 +0]*fFEvent[i*16 +0] + fFEvent[i*16 +1]*fFEvent[i*16 +1]);
                    fTemp1           = fTempCurrFric *-1.0f*fFEvent[i*16 +2];
                    ...
                    fTempCurrFric    = (uTemp0 == 1u)*(fTemp0/(-1.0f*fFEvent[i*16 +2])) + (uTemp0 != 1u)*fTempCurrFric ;
...
void     RotGlb2Loc(float fPrvStress[9], float fStressStk[6], float fStressDip[6], float fStressNrm[6], float fStk[3], float fDip[3], float fNrm[3])
{   //-------------------------------------------------------
    fPrvStress[0] = fNrm[0]*fStk[0]*fStressStk[0] + (fNrm[0]*fStk[1]+ fStk[0]*fNrm[1])*fStressStk[1] + (fNrm[0]*fStk[2] + fStk[0]*fNrm[2])*fStressStk[2] + (fStk[2]*fNrm[1] + fNrm[2]*fStk[1])*fStressStk[4] + fStk[1]*fNrm[1]*fStressStk[3] + fNrm[2]*fStk[2]*fStressStk[5]; //induced strike component
    fPrvStress[1] = fNrm[0]*fDip[0]*fStressStk[0] + (fNrm[0]*fDip[1]+ fDip[0]*fNrm[1])*fStressStk[1] + (fNrm[0]*fDip[2] + fDip[0]*fNrm[2])*fStressStk[2] + (fDip[2]*fNrm[1] + fNrm[2]*fDip[1])*fStressStk[4] + fDip[1]*fNrm[1]*fStressStk[3] + fNrm[2]*fDip[2]*fStressStk[5]; //induced dip component
    fPrvStress[2] = fNrm[0]*fNrm[0]*fStressStk[0] +              2.0f*fNrm[0]*fNrm[1] *fStressStk[1] +               2.0f*fNrm[0]*fNrm[2] *fStressStk[2] +               2.0f*fNrm[1]*fNrm[2] *fStressStk[4] + fNrm[1]*fNrm[1]*fStressStk[3] + fNrm[2]*fNrm[2]*fStressStk[5]; //induced normal component
    //-------------------------------------------------------
    fPrvStress[3] = fNrm[0]*fStk[0]*fStressDip[0] + (fNrm[0]*fStk[1]+ fStk[0]*fNrm[1])*fStressDip[1] + (fNrm[0]*fStk[2] + fStk[0]*fNrm[2])*fStressDip[2] + (fStk[2]*fNrm[1] + fNrm[2]*fStk[1])*fStressDip[4] + fStk[1]*fNrm[1]*fStressDip[3] + fNrm[2]*fStk[2]*fStressDip[5]; //induced strike component
    fPrvStress[4] = fNrm[0]*fDip[0]*fStressDip[0] + (fNrm[0]*fDip[1]+ fDip[0]*fNrm[1])*fStressDip[1] + (fNrm[0]*fDip[2] + fDip[0]*fNrm[2])*fStressDip[2] + (fDip[2]*fNrm[1] + fNrm[2]*fDip[1])*fStressDip[4] + fDip[1]*fNrm[1]*fStressDip[3] + fNrm[2]*fDip[2]*fStressDip[5]; //induced dip component
    fPrvStress[5] = fNrm[0]*fNrm[0]*fStressDip[0] +              2.0f*fNrm[0]*fNrm[1] *fStressDip[1] +               2.0f*fNrm[0]*fNrm[2] *fStressDip[2] +               2.0f*fNrm[1]*fNrm[2] *fStressDip[4] + fNrm[1]*fNrm[1]*fStressDip[3] + fNrm[2]*fNrm[2]*fStressDip[5]; //induced normal component
    //-------------------------------------------------------
    fPrvStress[6] = fNrm[0]*fStk[0]*fStressNrm[0] + (fNrm[0]*fStk[1]+ fStk[0]*fNrm[1])*fStressNrm[1] + (fNrm[0]*fStk[2] + fStk[0]*fNrm[2])*fStressNrm[2] + (fStk[2]*fNrm[1] + fNrm[2]*fStk[1])*fStressNrm[4] + fStk[1]*fNrm[1]*fStressNrm[3] + fNrm[2]*fStk[2]*fStressNrm[5]; //induced strike component
    fPrvStress[7] = fNrm[0]*fDip[0]*fStressNrm[0] + (fNrm[0]*fDip[1]+ fDip[0]*fNrm[1])*fStressNrm[1] + (fNrm[0]*fDip[2] + fDip[0]*fNrm[2])*fStressNrm[2] + (fDip[2]*fNrm[1] + fNrm[2]*fDip[1])*fStressNrm[4] + fDip[1]*fNrm[1]*fStressNrm[3] + fNrm[2]*fDip[2]*fStressNrm[5]; //induced dip component
    fPrvStress[8] = fNrm[0]*fNrm[0]*fStressNrm[0] +              2.0f*fNrm[0]*fNrm[1] *fStressNrm[1] +               2.0f*fNrm[0]*fNrm[2] *fStressNrm[2] +               2.0f*fNrm[1]*fNrm[2] *fStressNrm[4] + fNrm[1]*fNrm[1]*fStressNrm[3] + fNrm[2]*fNrm[2]*fStressNrm[5]; //induced normal component
    //-------------------------------------------------------
    return;
}
//----------------------------------------------------------------------------------------
