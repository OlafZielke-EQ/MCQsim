...
{   if ( (argc  > 3 ) || (argc < 2) ) {   fprintf(stdout,"Input Error\n Please start the code in the following way:\n\n mpirun -np 4 ./MCQsimV2 RunParaFile.txt");   exit(10);    }
    ...
    {   unsigned int *uTempF = calloc(uFPNum, sizeof *uTempF );
        ...
        {   fprintf(stdout,"----------------------\n");
            ...
            fprintf(stdout,"UseTime4RuptProp:       %u\n",uUseTimeProp);                    fprintf(stdout,"MinMag2UseRuptProp:     %f\n",fMinMag4Prop);
    ...
    fFF_S2RlocV  = calloc(iFOFFSET[iRANK], sizeof *fFF_S2RlocV );
    //---------------------------------------
    ...
    {   //--------------------------------------------------------------------------------
        ...
        {   timer0 = clock();
            ...
            float fTemp0,   fTemp1,   fTemp2,   fDist,   fAvDist;
            ...
            {   memset(fKh_FFBrch, 0,  23*uF_BrCount* sizeof(float));
                ...
                uTT_PS[i]       = calloc(   2*uFPNum, sizeof *uTT_PS[i] );
                fKh_FFvalstk[i] = calloc(   3*uFPNum, sizeof *fKh_FFvalstk[i] );
                fKh_FFvaldip[i] = calloc(   3*uFPNum, sizeof *fKh_FFvaldip[i] );
                fKh_FFvalnrm[i] = calloc(   3*uFPNum, sizeof *fKh_FFvalnrm[i] );
                fFF_S2RlocV[i]  = calloc(   3*uFPNum, sizeof *fFF_S2RlocV[i] );
                //--------------------------------
                for (j = 0u; j < uFPNum; j++)
                ...
                {   //--------------------------------
                    ...
                    {   uTemp0 = uF_OTElem[uF_MaxBrLvl*uSrcElem + (uF_MaxBrLvl-1u)]; //branchID at lowest level
                        uTT_PS[i][uKh_FFcnt[i]*2 +0]       = (unsigned int)( (fDist/ fModPara[5])              /fModPara[7] );
                        uTT_PS[i][uKh_FFcnt[i]*2 +1]       = (unsigned int)( (fDist/(fModPara[5]/fModPara[6])) /fModPara[7] );
                        fKh_FFvalstk[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uTemp0*23 +0];         fKh_FFvalstk[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uTemp0*23 +3];           fKh_FFvalstk[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uTemp0*23 +6];
                        fKh_FFvaldip[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uTemp0*23 +1];         fKh_FFvaldip[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uTemp0*23 +4];           fKh_FFvaldip[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uTemp0*23 +7];
                        fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uTemp0*23 +2];         fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uTemp0*23 +5];           fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uTemp0*23 +8];
                        //--------------------------------
                        fNrm[0]                            = fKh_FFBrch[uTemp0*23 +13];        fNrm[1]                            = fKh_FFBrch[uTemp0*23 +14];          fNrm[2]                            = fKh_FFBrch[uTemp0*23 +15];
                        fStk[0]                            = fKh_FFBrch[uTemp0*23 +16];        fStk[1]                            = fKh_FFBrch[uTemp0*23 +17];          fStk[2]                            = fKh_FFBrch[uTemp0*23 +18];
                        fDip[0]                            = fKh_FFBrch[uTemp0*23 +19];        fDip[1]                            = fKh_FFBrch[uTemp0*23 +20];          fDip[2]                            = fKh_FFBrch[uTemp0*23 +21];
                        //--------------------------------
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0]  = (fDist <= FLT_EPSILON) ? 0.0f : (fPrvSrcRcv[0]/fDist);
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1]  = (fDist <= FLT_EPSILON) ? 0.0f : (fPrvSrcRcv[1]/fDist);
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]  = (fDist <= FLT_EPSILON) ? 0.0f : (fPrvSrcRcv[2]/fDist);
                        //--------------------------------
                        fTemp0          = fNrm[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fNrm[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fNrm[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]; //this rotates the current source-to-receiver vector (in global coordinates)= fNrm[0]*fSrcRcvVect[0] + fNrm[1]*fSrcRcvVect[1] + fNrm[2]*fSrcRcvVect[2];
                        fTemp1          = fStk[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fStk[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fStk[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]; //into the local coordinate system of the source patch(that is the idea...)
                        fTemp2          = fDip[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fDip[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fDip[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2];
                        //--------------------------------
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0]  = fTemp1;//Source2Receiver vector in local coordinates; this is in strike
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1]  = fTemp2;//Source2Receiver vector in local coordinates; this is in dip
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]  = fTemp0;//Source2Receiver vector in local coordinates; this is in normal
                        uFUsdEl[uSrcElem]                  = 1u;
                    ...
                    {   //--------------------------------
                        ...
                        fSrc[0]       = fKh_FFBrch[uPrvBrId*23 +9];         fSrc[1]       = fKh_FFBrch[uPrvBrId*23 +10];        fSrc[2]       = fKh_FFBrch[uPrvBrId*23 +11];
                        fPrvSrcRcv[0] = fRcv[0]-fSrc[0];                    fPrvSrcRcv[1] = fRcv[1]-fSrc[1];                    fPrvSrcRcv[2] = fRcv[2]-fSrc[2];
                        fAvDist       = sqrtf(fPrvSrcRcv[0]*fPrvSrcRcv[0] + fPrvSrcRcv[1]*fPrvSrcRcv[1] + fPrvSrcRcv[2]*fPrvSrcRcv[2]);
                        //--------------------------------
                        ...
                        uTT_PS[i][uKh_FFcnt[i]*2 +0]       = (unsigned int)( (fAvDist/ fModPara[5])              /fModPara[7] );
                        uTT_PS[i][uKh_FFcnt[i]*2 +1]       = (unsigned int)( (fAvDist/(fModPara[5]/fModPara[6])) /fModPara[7] );
                        fKh_FFvalstk[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uPrvBrId*23 +0];         fKh_FFvalstk[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uPrvBrId*23 +3];           fKh_FFvalstk[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uPrvBrId*23 +6];
                        fKh_FFvaldip[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uPrvBrId*23 +1];         fKh_FFvaldip[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uPrvBrId*23 +4];           fKh_FFvaldip[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uPrvBrId*23 +7];
                        fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +0] = fKh_FFBrch[uPrvBrId*23 +2];         fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +1] = fKh_FFBrch[uPrvBrId*23 +5];           fKh_FFvalnrm[i][uKh_FFcnt[i]*3 +2] = fKh_FFBrch[uPrvBrId*23 +8];
                        //--------------------------------
                        fNrm[0]                            = fKh_FFBrch[uPrvBrId*23 +13];        fNrm[1]                            = fKh_FFBrch[uPrvBrId*23 +14];          fNrm[2]                            = fKh_FFBrch[uPrvBrId*23 +15]; 
                        fStk[0]                            = fKh_FFBrch[uPrvBrId*23 +16];        fStk[1]                            = fKh_FFBrch[uPrvBrId*23 +17];          fStk[2]                            = fKh_FFBrch[uPrvBrId*23 +18];
                        fDip[0]                            = fKh_FFBrch[uPrvBrId*23 +19];        fDip[1]                            = fKh_FFBrch[uPrvBrId*23 +20];          fDip[2]                            = fKh_FFBrch[uPrvBrId*23 +21];  
                        //--------------------------------
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0]  = fPrvSrcRcv[0]/fAvDist;             fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1]  = fPrvSrcRcv[1]/fAvDist;               fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]  = fPrvSrcRcv[2]/fAvDist; 
                        //--------------------------------
                        fTemp0          = fNrm[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fNrm[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fNrm[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]; //this rotates the current source-to-receiver vector (in global coordinates)= fNrm[0]*fSrcRcvVect[0] + fNrm[1]*fSrcRcvVect[1] + fNrm[2]*fSrcRcvVect[2];
                        fTemp1          = fStk[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fStk[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fStk[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]; //into the local coordinate system of the source patch(that is the idea...)
                        fTemp2          = fDip[0]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0] + fDip[1]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1] + fDip[2]*fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2];         
                        //--------------------------------
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +0]  = fTemp1;//Source2Receiver rector in local coordinates; this is in strike
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +1]  = fTemp2;//Source2Receiver rector in local coordinates; this is in dip
                        fFF_S2RlocV[i][uKh_FFcnt[i]*3 +2]  = fTemp0;//Source2Receiver rector in local coordinates; this is in normal
                        //--------------------------------
                        uUsdNum = 0u;
                        for (j = 0u; j < uFPNum;  j++)
                ...
                //------------------------------------------------------------------------
                uTT_PS[i]       = realloc(uTT_PS[i],       2*uKh_FFcnt[i]* sizeof *uTT_PS[i] );
                fKh_FFvalstk[i] = realloc(fKh_FFvalstk[i], 3*uKh_FFcnt[i]* sizeof *fKh_FFvalstk[i] );
                fKh_FFvaldip[i] = realloc(fKh_FFvaldip[i], 3*uKh_FFcnt[i]* sizeof *fKh_FFvaldip[i] );
                fKh_FFvalnrm[i] = realloc(fKh_FFvalnrm[i], 3*uKh_FFcnt[i]* sizeof *fKh_FFvalnrm[i] );
                fFF_S2RlocV[i]  = realloc(fFF_S2RlocV[i],  3*uKh_FFcnt[i]* sizeof *fFF_S2RlocV[i] );
                //------------------------------------------------------------------------
        ...
        {   float *FTempVect = calloc(iFOFFSET[iRANK], sizeof *FTempVect);
            ...
            for (i = 0u; i < iFOFFSET[iRANK]; i++)  {   lTemp0 += ( uKh_FFcnt[i]*( 2*sizeof(unsigned int) +12*sizeof(float) ) + uFPNum*sizeof(unsigned int));                   }
            ...
            {   MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),     uKh_FFps2[i],   uFPNum,       MPI_UNSIGNED,   &STATUS);         lLoclOffset += (uFPNum        *sizeof(unsigned int));
                MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),     uTT_PS[i],    2*uKh_FFcnt[i], MPI_UNSIGNED,   &STATUS);         lLoclOffset += (uKh_FFcnt[i]*2*sizeof(unsigned int));
                MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvalstk[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
                MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvaldip[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
                MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvalnrm[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
                MPI_File_write_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fFF_S2RlocV[i],  3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
            }
            OFFSETall += lTemp0;
            //----------------------------------------------------------------------------
            lTemp0 = 0;         memset(lRankOffset, 0, iSIZE*sizeof(unsigned long long ));
    ...
    {   float *FTempVect = calloc(iFOFFSET[iRANK], sizeof *FTempVect);
        ...
        for (i = 0u; i < iFOFFSET[iRANK]; i++) 
        {   uKh_FFps2[i]    = calloc(     uFPNum,       sizeof *uKh_FFps2[i] );
            uTT_PS[i]       = calloc(   2*uKh_FFcnt[i], sizeof *uTT_PS[i] );
            fKh_FFvalstk[i] = calloc(   3*uKh_FFcnt[i], sizeof *fKh_FFvalstk[i] );
            fKh_FFvaldip[i] = calloc(   3*uKh_FFcnt[i], sizeof *fKh_FFvaldip[i] );
            fKh_FFvalnrm[i] = calloc(   3*uKh_FFcnt[i], sizeof *fKh_FFvalnrm[i] );
            fFF_S2RlocV[i]  = calloc(   3*uKh_FFcnt[i], sizeof *fFF_S2RlocV[i] );
            lTemp0 += (uKh_FFcnt[i]*( 2*sizeof(unsigned int) + 12*sizeof(float) ) + uFPNum*sizeof(unsigned int));
        }
        lRankOffset[iRANK] = lTemp0;
        MPI_Allreduce(MPI_IN_PLACE, &lTemp0,       1,   MPI_UNSIGNED_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, lRankOffset, iSIZE, MPI_UNSIGNED_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);
        ...
        {   MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),     uKh_FFps2[i],   uFPNum,       MPI_UNSIGNED,   &STATUS);         lLoclOffset += (uFPNum  *sizeof(unsigned int));
            MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),     uTT_PS[i],    2*uKh_FFcnt[i], MPI_UNSIGNED,   &STATUS);         lLoclOffset += (uKh_FFcnt[i]*2*sizeof(unsigned int));
            MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvalstk[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
            MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvaldip[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
            MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fKh_FFvalnrm[i], 3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
            MPI_File_read_at(fp_KHMAT,  (OFFSETall + lRankStart[iRANK] + lLoclOffset),  fFF_S2RlocV[i],  3*uKh_FFcnt[i], MPI_FLOAT,      &STATUS);         lLoclOffset += (uKh_FFcnt[i]*3*sizeof(float));
        }
        OFFSETall += lTemp0;
        //-----------------------------------------------------------------------
        lTemp0 = 0;         memset(lRankOffset, 0, iSIZE*sizeof(unsigned long long ));
    ...
    float *fFslip    = calloc(3*uFPNum, sizeof *fFslip); //for slip in strike and dip
    ...
    //-------------------------------------------------------
    //-------------------------------------------------------
    unsigned int uMaxTTime = 0u;
    for (i = 0u; i < iFOFFSET[iRANK]; i++)
    {   for (j = 0u; j < uKh_FFcnt[i]; j++)
        {   uMaxTTime = (uMaxTTime > uTT_PS[i][j*2 +1])*uMaxTTime + (uMaxTTime <= uTT_PS[i][j*2 +1])*uTT_PS[i][j*2 +1];
    }   }
    //---------------------------------------
    MPI_Allreduce(MPI_IN_PLACE, &uMaxTTime, 1, MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
    //---------------------------------------
    float **fFF_StrMtrx = NULL;
    fFF_StrMtrx = calloc(iFOFFSET[iRANK], sizeof *fFF_StrMtrx );
    for (i = 0; i < iFOFFSET[iRANK]; i++)
    {   fFF_StrMtrx[i] = calloc(3*uMaxTTime, sizeof *fFF_StrMtrx[i] );            }
    //---------------------------------------
    ...
    {   unsigned int uGlobPos,   uTemp0,   uTemp1;
        ...
        {   memset(uSlipElCnt, 0,  2*sizeof(unsigned int));
            ...
            //----------------------------------------
            for (i = 0u; i < iFOFFSET[iRANK]; i++)
            {   memset(fFslip, 0, 3*uFPNum*sizeof(float) );
                ...
                {   uTemp0 = (unsigned int)fFslipG[j*3 +0]; //the global element ID of this slipping element
                    ...
                    fFslip[uTemp1*3 +0] += fFslipG[j*3 +1];
                    fFslip[uTemp1*3 +1] += fFslipG[j*3 +2];
                    fFslip[uTemp1*3 +2]  = 0.0f;
                }
                fFTempVal[i*5 +0] += cblas_sdot(3*uKh_FFcnt[i], fFslip, 1, fKh_FFvalstk[i], 1);
                fFTempVal[i*5 +1] += cblas_sdot(3*uKh_FFcnt[i], fFslip, 1, fKh_FFvaldip[i], 1);
    ...
    //------------------------------------------------------------------------------------
    //------------------------------------------------------------------------------------
    unsigned int uTemp0,   uTemp1,   uTemp2,   uTemp3,   uGlobPos,   uEQstillOn,   uTotlRuptT,   uActElmG,   uActElmL,   uMRFlgth;
    float fNxtLoadStep, fNxtHalfStep,   fTemp0,   fTemp1,   fTemp2,   fTemp3,    fTemp4,   fTemp5,   fTemp6,   fTemp7,   fTemp8,   fTemp9,   fTemp10,   fTemp11,    fTemp12;
    ...
    {   //--------------------------------------------------------------------------------
            ...
            {   //----------------------------------------
                ...
                {   for (j = 0u; j < uSlipElCnt[0]; j++) //going through all resolved sources i.e., using the slip they exhibit 
                    {   uTemp0 = (unsigned int)fFslipG[j*3 +0]; //the global element ID of this slipping element
                        ...
                        //--------------------------
                        fFTempVal[i*5 +0] += (fTemp0*fKh_FFvalstk[i][uTemp1*3 +0] + fTemp1*fKh_FFvalstk[i][uTemp1*3 +1]);
                        fFTempVal[i*5 +1] += (fTemp0*fKh_FFvaldip[i][uTemp1*3 +0] + fTemp1*fKh_FFvaldip[i][uTemp1*3 +1]);
                }   }
                else
                {   memset(fFslip, 0, 3*uFPNum*sizeof(float) );
                    for (j = 0u; j < uSlipElCnt[0]; j++) //going through all resolved sources i.e., using the slip they exhibit 
        ...
        {   timerI = clock() - timerI;        time_InterSeis += (((double)timerI)/CLOCKS_PER_SEC);
            ...
            {   //-----------------------
                ...
                //-----------------------
                fTemp0 = (fFFric[i*6 +1] *-1.0f *fFEvent[i*16 +2]) + fFRef[i*14 +2]*fFEvent[i*16 +7]; //this is arrest strength plus the stress/strength needed to achieve D_c slip (use arrest, b/c D_c was adjusted accordingly)
                ...
                {   if (uEQstillOn < uKh_FFcnt[i])
                    {   for (j = 0u; j < uEQstillOn; j++)
                        {   uTemp0 = (unsigned int)fFslipG[j*3 +0]; //the global element ID of this slipping element
                            ...
                            //-------------------------------
                            fFEvent[i*16 +0] += (fTemp0*fKh_FFvalstk[i][uTemp1*3 +0] + fTemp1*fKh_FFvalstk[i][uTemp1*3 +1]);
                            fFEvent[i*16 +1] += (fTemp0*fKh_FFvaldip[i][uTemp1*3 +0] + fTemp1*fKh_FFvaldip[i][uTemp1*3 +1]);
                            fFEvent[i*16 +2] += (fTemp0*fKh_FFvalnrm[i][uTemp1*3 +0] + fTemp1*fKh_FFvalnrm[i][uTemp1*3 +1]);
                    }   }
                    else
                    {   memset(fFslip, 0, 3*uFPNum*sizeof(float) );
                        ...
                        fFEvent[i*16 +0] += cblas_sdot(3*uKh_FFcnt[i], fFslip, 1, fKh_FFvalstk[i], 1);
                        fFEvent[i*16 +1] += cblas_sdot(3*uKh_FFcnt[i], fFslip, 1, fKh_FFvaldip[i], 1);
                        fFEvent[i*16 +2] += cblas_sdot(3*uKh_FFcnt[i], fFslip, 1, fKh_FFvalnrm[i], 1);
                }   }
            ...
            //----------------------------------------------------------------------------
            //----------------------------------------------------------------------------
            if ((fEQMagn >= fMinMag4Prop) && (uUseTimeProp == 1u))
            {   if ((uPlotCatalog2Screen == 1u) && (iRANK == 0u))     {   fprintf(stdout,"M_t %3.2f  ",fEQMagn);          }
                uEQstillOn = 1u;        uMRFlgth = 0u;            uActElmL = 0u;         uTotlRuptT = 0u;                  fEQMomRel = 0.0f;
                memset(uFActivEl,    0,      iFOFFSET[iRANK]*sizeof(unsigned int) );
                memset(fMRFvals,     0, MAXMOMRATEFUNCLENGTH*sizeof(float) );
                memset(fSTFslip,     0, 2*iFOFFSET[iRANK]*MAXMOMRATEFUNCLENGTH*sizeof(float));
                memset(fFF_StrComb,  0,    3*iFOFFSET[iRANK]*sizeof(float) );
                for (i = 0; i < iFOFFSET[iRANK]; i++)   {       memset(fFF_StrMtrx[i],  0, 3*uMaxTTime*sizeof(float));     }
                //----------------------------------------------------------------
                for (i = 0; i < iFOFFSET[iRANK]; i++)
                {   fFEvent[i*16 +0] = fFTempVal[i*5 +0];       fFEvent[i*16 +1] = fFTempVal[i*5 +1];       fFEvent[i*16 +2] = fFTempVal[i*5 +2];       fFEvent[i*16 +3] = fFTempVal[i*5 +3];
                    ...
                    fFFric[i*6 +3]   = fFEvent[i*16 +3]; //the temprefFriction from which I decay from
                    fFFric[i*6 +5]   = fFEvent[i*16 +3]; //the temprefFriction to which I will heal
                }//finish resetting all to pre-EQ state
                //----------------------------------------------------------------
                while (uEQstillOn > 0u)
                {   uEQstillOn  = 0u;
                    memset(iStartPosF, 0, iSIZE*sizeof(int) );           memset(iOffstPosF, 0, iSIZE*sizeof(int) );
                    ...
                    {   for (j = 0u; j < uEQstillOn; j++)
                        {   uTemp0 = (unsigned int)fFslipG[j*3 +0]; //the global element ID of this slipping element
                            ...
                            fTemp1 = fFslipG[j*3 +2]; //dip slip
                            //-------------------------------
                            uTemp2 = (uTotlRuptT + uTT_PS[i][uTemp1*2 +0]) % uMaxTTime;// current time plus travel time of P signal
                            uTemp3 = (uTotlRuptT + uTT_PS[i][uTemp1*2 +1]) % uMaxTTime;// current time plus travel time of S signal
                            //-------------------------------
                            fTemp3 = fTemp0*fFF_S2RlocV[i][uTemp1*3 +0] + fTemp1*fFF_S2RlocV[i][uTemp1*3 +1]; //this is the length of slip vector in SrcRcv_vector direction (in P-"wave" direction); THE src-rcv-entries are zero if distance between them was zero (i.e., on "self")
                            //------------------------------
                            fTemp4 = (uTemp2 == uTemp3)*0.0f   + (uTemp2 != uTemp3)*(fTemp0 - (fFF_S2RlocV[i][uTemp1*3 +0]*fTemp3)); //rejection -in strike-direction
                            fTemp5 = (uTemp2 == uTemp3)*0.0f   + (uTemp2 != uTemp3)*(fTemp1 - (fFF_S2RlocV[i][uTemp1*3 +1]*fTemp3)); //rejection -in dip-direction
                            fTemp6 = (uTemp2 == uTemp3)*0.0f   + (uTemp2 != uTemp3)*(0.0f   - (fFF_S2RlocV[i][uTemp1*3 +2]*fTemp3)); //rejection -in normal-direction
                            //-------------------------------
                            fTemp0 = (uTemp2 == uTemp3)*fTemp0 + (uTemp2 != uTemp3)*(          fFF_S2RlocV[i][uTemp1*3 +0]*fTemp3); //projection -in strike-direction
                            fTemp1 = (uTemp2 == uTemp3)*fTemp1 + (uTemp2 != uTemp3)*(          fFF_S2RlocV[i][uTemp1*3 +1]*fTemp3); //projection -in strike-direction
                            fTemp2 = (uTemp2 == uTemp3)*0.0f   + (uTemp2 != uTemp3)*(          fFF_S2RlocV[i][uTemp1*3 +2]*fTemp3); //projection -in strike-direction
                            //-------------------------------
                            fTemp7 = (fTemp0*fKh_FFvalstk[i][uTemp1*3 +0] + fTemp1*fKh_FFvalstk[i][uTemp1*3 +1] + fTemp2*fKh_FFvalstk[i][uTemp1*3 +2]); //P-induced stress change in stk-dir
                            fTemp10= (fTemp4*fKh_FFvalstk[i][uTemp1*3 +0] + fTemp5*fKh_FFvalstk[i][uTemp1*3 +1] + fTemp6*fKh_FFvalstk[i][uTemp1*3 +2]); //S-induced stress change in stk-dir
                            fTemp8 = (fTemp0*fKh_FFvaldip[i][uTemp1*3 +0] + fTemp1*fKh_FFvaldip[i][uTemp1*3 +1] + fTemp2*fKh_FFvaldip[i][uTemp1*3 +2]); //P-induced stress change in dip-dir
                            fTemp11= (fTemp4*fKh_FFvaldip[i][uTemp1*3 +0] + fTemp5*fKh_FFvaldip[i][uTemp1*3 +1] + fTemp6*fKh_FFvaldip[i][uTemp1*3 +2]); //S-induced stress change in dip-dir
                            fTemp9 = (fTemp0*fKh_FFvalnrm[i][uTemp1*3 +0] + fTemp1*fKh_FFvalnrm[i][uTemp1*3 +1] + fTemp2*fKh_FFvalnrm[i][uTemp1*3 +2]); //P-induced stress change in nrm-dir
                            fTemp12= (fTemp4*fKh_FFvalnrm[i][uTemp1*3 +0] + fTemp5*fKh_FFvalnrm[i][uTemp1*3 +1] + fTemp6*fKh_FFvalnrm[i][uTemp1*3 +2]); //S-induced stress change in nrm-dir
                            //-------------------------------
                            fFF_StrMtrx[i][uTemp2*3 +0] += fTemp7;
                            fFF_StrMtrx[i][uTemp2*3 +1] += fTemp8;
                            fFF_StrMtrx[i][uTemp2*3 +2] += fTemp9;
                            fFF_StrMtrx[i][uTemp3*3 +0] += fTemp10;
                            fFF_StrMtrx[i][uTemp3*3 +1] += fTemp11;
                            fFF_StrMtrx[i][uTemp3*3 +2] += fTemp12;
                            //-------------------------------
                            fFF_StrComb[i*3 +0] += (fTemp7 + fTemp10);
                            fFF_StrComb[i*3 +1] += (fTemp8 + fTemp11);
                            fFF_StrComb[i*3 +2] += (fTemp9 + fTemp12);
                        }
                        //-------------------------------------------------
                        uTemp2 = uTotlRuptT % uMaxTTime;//modulus for current time
                        //-------------------------------------------------
                        fTemp0 = fFF_StrMtrx[i][uTemp2*3 +0];                   fFF_StrMtrx[i][uTemp2*3 +0] = 0.0f;//current stress change in strike
                        fTemp1 = fFF_StrMtrx[i][uTemp2*3 +1];                   fFF_StrMtrx[i][uTemp2*3 +1] = 0.0f;//current stress change in dip
                        fTemp2 = fFF_StrMtrx[i][uTemp2*3 +2];                   fFF_StrMtrx[i][uTemp2*3 +2] = 0.0f;//current stress change in normal
                        //-------------------------------
                        fFEvent[i*16 +0] += fTemp0;                             fFF_StrComb[i*3 +0] -= fTemp0;
                        fFEvent[i*16 +1] += fTemp1;                             fFF_StrComb[i*3 +1] -= fTemp1;
                        fFEvent[i*16 +2] += fTemp2;                             fFF_StrComb[i*3 +2] -= fTemp2;
                        //-------------------------------------------------
                    }
                    //-------------------------------------------------
                    if (uEQstillOn == 0u)
                    {   for (i = 0u; i < iFOFFSET[iRANK]; i++)
                        {   fTemp0 = fFEvent[i*16 +0] + fFF_StrComb[i*3 +0]; //currently applied shear stress in stk-dir plus total change (in shear stress in stk-dir) currently still in the system
                            fTemp1 = fFEvent[i*16 +1] + fFF_StrComb[i*3 +1];
                            fTemp3 = sqrtf(fTemp0*fTemp0 + fTemp1*fTemp1); //combined shear stress on element
                            //----------------------------------------------------
                            fTemp2 = fFEvent[i*16 +2] + fFF_StrComb[i*3 +2];
                            fTemp2 = (fTemp2 < 0.0f)*fTemp2 + (fTemp2 >= 0.0f)*0.0f; 
                            //----------------------------------------------------
                            fTemp4 = fFEvent[i*16 +3] *-1.0f*fTemp2; //current fault strength
                            //----------------------------------------------------
                            uEQstillOn = (fTemp3 > fTemp4)*(uEQstillOn +1u) + (fTemp3 <= fTemp4)*uEQstillOn;
                        }
                        //-----------------------------------------------------------------------
                        MPI_Allreduce(MPI_IN_PLACE, &uEQstillOn, 1, MPI_UNSIGNED, MPI_SUM, MPI_COMM_WORLD);
                        //----------------------------------------------------
                        if (uEQstillOn == 0u)
                        {   for (i = 0u; i < iFOFFSET[iRANK]; i++)
                            {   fFEvent[i*16 +0] += fFF_StrComb[i*3 +0];
                                fFEvent[i*16 +1] += fFF_StrComb[i*3 +1];
                                fFEvent[i*16 +2] += fFF_StrComb[i*3 +2];
                    }   }   }
                    //---------------------------------
                    uEQstillOn = (uTotlRuptT < MAXMOMRATEFUNCLENGTH-1)*uEQstillOn + (uTotlRuptT >= MAXMOMRATEFUNCLENGTH-1)*0u; //hard stop
                    uTotlRuptT += 1u;
